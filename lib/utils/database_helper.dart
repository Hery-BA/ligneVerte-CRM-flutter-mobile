...
//============UPDATEPROJECT=============================
  ...
  Future<int> updateRegion(RegionModel region) async {
    ...
    return db!.update(RegionModel.tableRegion, region.toMap(),
        where: '${RegionModel.coloneId}=?', whereArgs: [region.idreg]);
  }

  //===============DELETEPROJECT==========================
  Future<int> delete(int id) async {
    Database? db = await database;
    return await db!.delete(ProjectModel.tableProject,
        where: '${ProjectModel.coloneId}=?', whereArgs: [id]);
  }
...
//======================DELETE REGION================================`
  Future<int> deleteRegion(int id) async {
    Database? db = await database;
    return db!.delete(RegionModel.tableRegion,
        where: '${RegionModel.coloneId}=?', whereArgs: [id]);
  }
  ...
  //======================AFFICHE REGION================================
  Future<List<RegionModel>> afficherRegion() async {
    Database? db = await database;
    List<Map> allregion = await db!.query(RegionModel.tableRegion);
    return allregion.length == 0
        ? []
        : allregion
